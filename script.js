...
function detectHeartImages(maxCheck = 50) {
  // Auto-detects images named images/img1.jpg, images/img2.jpg, images/img3.jpg, ...
  // Stops at the first missing number, so just drop new files in sequence
  // (no need to edit this file when adding/removing photos).
  const found = [];
  for (let i = 1; i <= maxCheck; i++) {
    const path = `images/img${i}.jpg`;
    try {
      const xhr = new XMLHttpRequest();
      xhr.open('HEAD', `${path}?v=${Date.now()}`, false); // cache-buster avoids stale 404s
      xhr.send();
...
const defaultHeartImages = detectHeartImages();
...
const ringTexts = [
  ...
  "28/06/2026",//untuk deskripsi planet layer 4
...
function onCanvasClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    playGalaxyAudio();
    if (window.startGalaxyMusic) window.startGalaxyMusic();
